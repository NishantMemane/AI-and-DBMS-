...
#     # ADDED KEYWORDS: "explain my incomes", "received total", "explain my income"
#     if any(k in t for k in ["summary", "balance", "total", "how much", "spent on", "how much i spent", "top spending", "top expense", "breakdown", "expense total", "income total", "explain my incomes", "explain my income", "received total"]):

#         # Check for specific category query
#         cat = parse_category(text)
#         if ("spent on" in t or ("how much" in t and cat and ("expense" in t or "spent" in t))):
#             if not cat:
...
#         # --- NEW: Handle specific income query (like "explain my incomes") ---
...
#             append_chat(user_id, "assistant", reply)
#             return {"text": reply, "chart_data": None} # Return without chart data
#         # --- END NEW INCOME HANDLER ---


#         # General summary (Fall through for queries like "summary" or "balance")
#         s = compute_summary(user_id)
#         breakdown = _sql_expense_breakdown(user_id)
...
#         reply = context_text if not (GENIE_KEY and genai) else _gemini_rewrite(user_id, text, context_text)
#         append_chat(user_id, "assistant", reply)
#         return {"text": reply, "chart_data": chart_data} # <-- RETURN chart_data

#     # 3. Add Expense intent (Data Entry Intent)
#     if any(k in t for k in ["spent", "bought", "paid", "expense", "purchase", "paid for"]):
...
#             ask = f"I can add this income. I still need {', '.join(missing)}. Please reply with the missing info (e.g., 'from Salary')."
#             append_chat(user_id, "assistant", ask)
#             return {"text": ask, "chart_data": None}
...
#         "2. **Checking Data**: Ask 'What is my balance?' or 'How much did I spend on Transport?' or 'Explain my incomes'.\n" # Updated guidance
